...
echo '0 4 1 * * root /opt/node/docker/testnet/scripts/sync-cloudflare-ufw.sh 2>&1 | logger -t cf-ufw' \
...
The `docker-compose.rpc.yml` stack runs the node **and Caddy**. Caddy is a custom
...
```bash
docker compose -f docker-compose.rpc.yml build caddy   # compile the rate-limit plugin
docker compose -f docker-compose.rpc.yml pull          # node image
...
> for the RPC hostnames (skip managed rules + Super Bot Fight Mode). See
> `../../docs/rpc-node-setup.md`.
