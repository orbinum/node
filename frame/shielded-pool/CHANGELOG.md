...
- **Weights regenerated** — the larger memo grows `CommitmentMemos`' `MaxEncodedLen`, changing the `proof_size` of every extrinsic that reads/writes it (`shield`, `shield_batch`, `private_transfer`, `unshield`). Runtime `spec_version` 5 → 6.
