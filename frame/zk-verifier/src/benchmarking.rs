...
mod benchmarks {
	...
	/// A real BN254 Groth16 VK for the TRANSFER circuit (arity 5), serialized as the
	/// pallet consumes it. Registration validates the VK deserializes and matches the
	/// circuit arity, so the storage benchmarks need a well-formed key, not a byte
	/// pattern.
	fn sample_verification_key() -> Vec<u8> {
		use ark_bn254::{Bn254, G1Affine, G2Affine};
		use ark_ec::AffineRepr;
		use ark_groth16::VerifyingKey as ArkVk;
		use orbinum_zk_verifier::{TRANSFER_PUBLIC_INPUTS, VerifyingKey};
		...
		let vk = ArkVk::<Bn254> {
			...
			gamma_abc_g1: (0..=TRANSFER_PUBLIC_INPUTS)
				.map(|_| G1Affine::generator())
				.collect(),
		};
		VerifyingKey::from_ark_vk(&vk)
			.expect("benchmark VK serializes")
			.bytes
	}
