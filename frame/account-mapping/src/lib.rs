...
pub mod pallet {
    ...
    use frame_support::sp_runtime::traits::{Dispatchable, Zero, Convert};
    ...
    pub enum Event<T: Config> {
        ...
        AccountMapped { account: T::AccountId, address: H160 },
        /// A stateful EVM mapping was removed.
        AccountUnmapped { account: T::AccountId, address: H160 },
        /// A human-readable alias was registered.
        AliasRegistered { account: T::AccountId, alias: AliasOf<T>, evm_address: Option<H160> },
        /// An alias was released and the deposit returned.
        AliasReleased { account: T::AccountId, alias: AliasOf<T> },
        ...
        AliasSaleCancelled { seller: T::AccountId, alias: AliasOf<T> },
        ...
        ChainLinkAdded { account: T::AccountId, chain_id: ChainId, address: ExternalAddr },
        /// A chain link was removed.
        ChainLinkRemoved { account: T::AccountId, chain_id: ChainId },
        /// Account metadata (profile) was updated.
        MetadataUpdated { account: T::AccountId },
        /// A new chain support was added by governance.
        SupportedChainAdded { chain_id: ChainId, scheme: SignatureScheme },
        ...
        ProxyCallExecuted { owner: T::AccountId, chain_id: ChainId, address: ExternalAddr },
        /// A private chain link commitment was registered. Address never revealed.
        PrivateChainLinkAdded { account: T::AccountId, chain_id: ChainId, commitment: [u8; 32] },
        /// A private chain link commitment was removed by its owner.
        PrivateChainLinkRemoved { account: T::AccountId, chain_id: ChainId, commitment: [u8; 32] },
        ...
        PrivateChainLinkRevealed { account: T::AccountId, chain_id: ChainId, address: ExternalAddr },
        ...
        PrivateLinkDispatchExecuted { owner: T::AccountId, commitment: [u8; 32] },
    }
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn map_account(origin: OriginFor<T>) -> DispatchResult {
            ...
            ensure!(!OriginalAccounts::<T>::contains_key(&who), Error::<T>::AlreadyMapped);

            let address = T::AccountIdToEvmAddress::convert(who.clone())
                .ok_or(Error::<T>::NativeAccountCannotBeMapped)?;
            ensure!(!MappedAccounts::<T>::contains_key(address), Error::<T>::AddressAlreadyMapped);

            OriginalAccounts::<T>::insert(&who, address);
            MappedAccounts::<T>::insert(address, &who);

            Self::deposit_event(Event::AccountMapped { account: who, address });
        ...
        pub fn register_alias(origin: OriginFor<T>, alias: AliasOf<T>) -> DispatchResult {
            ...
            ensure!(!AccountAliases::<T>::contains_key(&who), Error::<T>::AlreadyHasAlias);

            // Alias must be unclaimed.
            ensure!(!Identities::<T>::contains_key(&alias), Error::<T>::AliasTaken);
        ...
        pub fn release_alias(origin: OriginFor<T>) -> DispatchResult {
            ...
            Self::deposit_event(Event::AliasReleased { account: who, alias });
        ...
        pub fn put_alias_on_sale(
            ...
            ensure!(!AliasListings::<T>::contains_key(&alias), Error::<T>::AlreadyForSale);

            // Convert Vec to BoundedVec (enforces MAX_WHITELIST_SIZE).
            let bounded_buyers: Option<BoundedVec<T::AccountId, ConstU32<{ crate::MAX_WHITELIST_SIZE }>>> =
                match allowed_buyers {
                    None => None,
                    Some(v) => Some(
                        v.try_into().map_err(|_| Error::<T>::WhitelistTooLarge)?
                    ),
                };

            let private = bounded_buyers.is_some();
            let listing = SaleListing { price, allowed_buyers: bounded_buyers };

            AliasListings::<T>::insert(&alias, listing);

            Self::deposit_event(Event::AliasListedForSale { seller: who, alias, price, private });
            Ok(())
        }
        ...
        pub fn cancel_sale(origin: OriginFor<T>) -> DispatchResult {
            ...
            ensure!(AliasListings::<T>::contains_key(&alias), Error::<T>::NotForSale);
        ...
        pub fn buy_alias(origin: OriginFor<T>, alias: AliasOf<T>) -> DispatchResult {
            ...
            ensure!(!AccountAliases::<T>::contains_key(&buyer), Error::<T>::BuyerAlreadyHasAlias);
            ...
            T::Currency::transfer(&buyer, &seller, listing.price, ExistenceRequirement::KeepAlive)?;
            ...
            Self::deposit_event(Event::AliasSold { seller, buyer, alias, price: listing.price });
            Ok(())
        }
        ...
        pub fn add_chain_link(
            ...
            // Add the link.
            let bounded_addr: ExternalAddr = address.clone().try_into()
                .map_err(|_| Error::<T>::InvalidAliasCharacters)?; // Max length exceeded

            record.chain_links.try_push(ChainLink {
                chain_id,
            ...
            Self::deposit_event(Event::ChainLinkAdded { account: who, chain_id, address: bounded_addr });
            Ok(())
        }
        ...
        pub fn remove_chain_link(origin: OriginFor<T>, chain_id: ChainId) -> DispatchResult {
            ...
            let pos = record.chain_links.iter().position(|l| l.chain_id == chain_id)
            ...
            Self::deposit_event(Event::ChainLinkRemoved { account: who, chain_id });
            Ok(())
        }
        ...
        pub fn set_account_metadata(
            ...
            let metadata = AccountMetadata { display_name, bio, avatar };
        ...
        pub fn dispatch_as_linked_account(
            ...
            let result = call.dispatch_bypass_filter(frame_system::RawOrigin::Signed(owner.clone()).into());
            
            let dispatch_result = result.map(|_| ()).map_err(|e| e.error);

            dispatch_result?;

            Self::deposit_event(Event::ProxyCallExecuted { owner, chain_id, address: bounded_addr });
            Ok(())
        }
        ...
        pub fn register_private_link(
            ...
            // Reject duplicate chain_id or identical commitment.
                ...
                !links.iter().any(|l| l.chain_id == chain_id || l.commitment == commitment),
                Error::<T>::PrivateLinkAlreadyExists
            );

            links.try_push(PrivateChainLink { chain_id, commitment })
                .map_err(|_| Error::<T>::TooManyPrivateLinks)?;

            PrivateChainLinks::<T>::insert(&alias, links);
            Self::deposit_event(Event::PrivateChainLinkAdded { account: who, chain_id, commitment });
            Ok(())
        }
        ...
        pub fn remove_private_link(
            ...
            let pos = links.iter().position(|l| l.commitment == commitment)
        ...
        pub fn reveal_private_link(
            ...
            let pos = private_links.iter().position(|l| l.commitment == commitment)
                .ok_or(Error::<T>::PrivateLinkNotFound)?;
            ...
            match scheme {
                SignatureScheme::Eip191 => {
                    let sig_array: [u8; 65] = signature
                        ...
                        .map_err(|_| Error::<T>::InvalidSignature)?;
                    crate::utils::verify_ethereum_signature(
                        address.as_slice(), &who.encode(), &sig_array,
                    ).map_err(|_| Error::<T>::InvalidSignature)?;
                },
                SignatureScheme::Ed25519 => {
                    let sig_array: [u8; 64] = signature
            ...
            record.chain_links.try_push(ChainLink { chain_id, address: bounded_addr.clone() })
                .map_err(|_| Error::<T>::TooManyLinks)?;
