...
fn make_alias<T: Config>(seed: &[u8]) -> AliasOf<T> {
    ...
    bytes.truncate(14);
    bytes.try_into().expect("14-byte alias always fits within MaxAliasLength ≥ 14")
}
...
mod benchmarks {
    ...
    fn cancel_sale() {
        ...
        let price = T::AliasDeposit::get() * 10u32.into();
        let listing = crate::pallet::SaleListing::<T> { price, allowed_buyers: None };
    ...
    fn add_chain_link() {
        ...
        let eth_addr: crate::ExternalAddr = addr_hash[12..].to_vec().try_into()
            .expect("20 bytes always fit in ExternalAddr");

        #[extrinsic_call]
        add_chain_link(RawOrigin::Signed(caller.clone()), 1u32, eth_addr.into_inner(), sig_bytes.to_vec());
    ...
    fn remove_chain_link() {
        ...
        record.chain_links.try_push(crate::ChainLink {
            chain_id: 1,
            address: link_addr,
    ...
    fn set_account_metadata() {
        ...
        let display_name: Option<frame_support::BoundedVec<u8, frame_support::pallet_prelude::ConstU32<{ crate::MAX_METADATA_LEN }>>> =
            Some(b"Benchmark User".to_vec().try_into().expect("short name"));
        let bio: Option<frame_support::BoundedVec<u8, frame_support::pallet_prelude::ConstU32<{ crate::MAX_METADATA_LEN }>>> =
            Some(b"Benchmark bio string for testing".to_vec().try_into().expect("short bio"));
    ...
    fn dispatch_as_linked_account() {
        ...
        let pubkey = sp_io::crypto::ecdsa_generate(BENCH_KEY, Some(b"//bench_dispatch_01".to_vec()));
        ...
        let full_pubkey = sp_io::crypto::secp256k1_ecdsa_recover(&sig_bytes, &final_hash)
            .ok().expect("valid recovery");
        let addr_hash = sp_io::hashing::keccak_256(&full_pubkey[..]);
        let eth_addr: crate::ExternalAddr = addr_hash[12..]
            .to_vec().try_into().expect("20 bytes fit in ExternalAddr");
        ...
        let inner_call: <T as crate::pallet::Config>::RuntimeCall =
            frame_system::Call::<T>::remark { remark: Default::default() }.into();
    ...
    fn remove_private_link() {
        ...
        let link = crate::PrivateChainLink { chain_id: 502u32, commitment };
    ...
    fn reveal_private_link() {
        ...
        let link = crate::PrivateChainLink { chain_id: 1u32, commitment };
    ...
    fn dispatch_as_private_link() {
        ...
        let link = crate::PrivateChainLink { chain_id: 503u32, commitment };
        let mut prv_links = crate::pallet::PrivateChainLinks::<T>::get(&alias);
        prv_links.try_push(link).expect("free slot");
        crate::pallet::PrivateChainLinks::<T>::insert(&alias, prv_links);
