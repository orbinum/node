...
pub trait AccountMappingApi<BlockHash> {
    ...
    fn get_account_metadata(&self, account_id: String) -> RpcResult<Option<AccountMetadataResponse>>;
    ...
    fn get_supported_chains(&self) -> RpcResult<Vec<(u32, pallet_account_mapping_runtime_api::SignatureScheme)>>;
...
fn parse_h160(hex_address: &str) -> Result<H160, ErrorObjectOwned> {
    let bytes = hex::decode(hex_address.trim_start_matches("0x"))
        .map_err(error::invalid_h160)?;
    if bytes.len() != 20 {
...
fn parse_account_id32(hex_account: &str) -> Result<AccountId32, ErrorObjectOwned> {
    let bytes = hex::decode(hex_account.trim_start_matches("0x"))
        .map_err(error::invalid_account_id)?;
    if bytes.len() != 32 {
        return Err(error::invalid_account_id("AccountId must be exactly 32 bytes"));
    }
...
fn alias_info_to_response(alias: String, info: AliasInfo<AccountId32>) -> AliasResponse {
    ...
    let evm_hex = info.evm_address.map(|a| format!("0x{}", hex::encode(a.as_bytes())));
    AliasResponse {
        alias,
    ...
    fn resolve_full_identity(&self, alias: String) -> RpcResult<Option<FullIdentityResponse>> {
        ...
        Ok(info.map(|i| {
            ...
            FullIdentityResponse {
                ...
                evm_address: i.evm_address.map(|a| format!("0x{}", hex::encode(a.as_bytes()))),
                chain_links: i.chain_links.into_iter().map(|l| ChainLinkResponse {
                    chain_id: l.chain_id,
...
;
        if bytes.len() != 32 {
            return Err(error::invalid_chain_address("commitment must be exactly 32 bytes"));
        }
...
mod tests {
    ...
    fn parse_account_id32_accepts_valid_hex() {
        let parsed =
            parse_account_id32("0x0101010101010101010101010101010101010101010101010101010101010101")
                .unwrap();
