...
where
    ...
    <<T as frame_system::Config>::RuntimeCall as Dispatchable>::RuntimeOrigin: From<Option<<T as frame_system::Config>::AccountId>>,
    ...
    pallet_evm::AccountIdOf<T>: Into<<T as frame_system::Config>::AccountId>,
{
    ...
    fn register_alias(handle: &mut impl PrecompileHandle, input: &[u8]) -> PrecompileResult {
        ...
        let length = U256::from_big_endian(&params[abs_len_pos..abs_len_pos + 32]).low_u32() as usize;
        ...
        let bounded_alias: pallet_account_mapping::AliasOf<T> =
            alias_bytes.try_into().map_err(|_| PrecompileFailure::Error {
                exit_status: ExitError::Other("alias too long".into()),
            })?;

        let call = <<T as frame_system::Config>::RuntimeCall as From<pallet_account_mapping::Call<T>>>::from(
            pallet_account_mapping::Call::<T>::register_alias { alias: bounded_alias },
        );
        ...
        let dispatch_origin =
            <<T as frame_system::Config>::RuntimeCall as Dispatchable>::RuntimeOrigin::from(Some(origin));
    ...
    fn resolve_alias(input: &[u8]) -> PrecompileResult {
        ...
        let (owner_h160, evm_h160) =
            if let Some(record) = pallet_account_mapping::Pallet::<T>::runtime_api_resolve_alias(&alias_bytes) {
                let owner_h160 = record.evm_address
                    .unwrap_or(sp_core::H160::zero());
    ...
    fn get_alias_of(input: &[u8]) -> PrecompileResult {
        ...
        let alias: Vec<u8> =
            pallet_account_mapping::Pallet::<T>::runtime_api_get_alias_of(account)
                .unwrap_or_default();
