...
# crates we pin (49.0.0 / 58.0.0), and `pallet-ismp` pins `polkadot-sdk =2606.0.0`
...
FB="$VENDOR_DIR/frame-benchmarking-49.0.0"
CLI="$VENDOR_DIR/frame-benchmarking-cli-58.0.0"
for d in "$FB" "$CLI"; do
    [[ -d "$d" ]] || { echo "ERROR: expected $d after vendoring" >&2; exit 1; }
done
...
# targets master, we build the published 49.0.0/58.0.0), hence -F3.
#
...
apply_one() {
    ...
    then
        ...
        echo "   FAILED to apply $label against 49.0.0/58.0.0." >&2
