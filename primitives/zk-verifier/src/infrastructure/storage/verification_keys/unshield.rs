...
pub fn get_vk() -> VerifyingKey<Bn254> {
	...
	let delta_g2 = G2Affine::new_unchecked(
		Fq2::new(
			Fq::from_str(
				"5366741149662063188266137377962915507572865199781677781989129371421156725687",
			)
			.unwrap(),
			Fq::from_str(
				"6681995908313451802157835590696574494457884096128656590141338524934422338374",
			)
			.unwrap(),
		),
		Fq2::new(
			Fq::from_str(
				"12379514915304873009905993938394648346383109249883866397023842744782617281064",
			)
			.unwrap(),
			Fq::from_str(
				"9530901712062734541139784071987871413548881368245087348176540295539151383884",
			)
			.unwrap(),
		),
	);
