...
pub fn get_vk() -> VerifyingKey<Bn254> {
	...
	let delta_g2 = G2Affine::new_unchecked(
		Fq2::new(
			Fq::from_str(
				"16287772643505201893247688978023470035587350997446916996173005135148124296113",
			)
			.unwrap(),
			Fq::from_str(
				"2926337104520321001596843200372791604819321073724722170601520363087971026934",
			)
			.unwrap(),
		),
		Fq2::new(
			Fq::from_str(
				"6132240034673832099124816799541693941623520680420608218330147309563540590990",
			)
			.unwrap(),
			Fq::from_str(
				"2594972947204383133505845599839316076395437909755842461014039519624174403526",
			)
			.unwrap(),
		),
	);
